...
- 仓库主体为前端应用，同时包含 Cloudflare Worker 接口与静态资源一体化部署链路。
...
- `IndexedDB` 本地持久化
- `fflate` 云同步压缩与解压
- Cloudflare Worker + Static Assets 接口
...
- `VITE_SYNC_API_BASE_URL` 用于 APK / 原生壳把同步请求指向你部署好的 Cloudflare Worker 域名；纯网页部署可留空。

### GitHub OAuth 部署检查

如果你使用 `.github/workflows/deploy-worker.yml` 通过 GitHub Actions 构建再部署到 Cloudflare Worker，需要同时配置下面两类变量：

- GitHub Actions Secret：`VITE_GITHUB_CLIENT_ID`
- Cloudflare Worker 运行时变量：`GITHUB_CLIENT_ID`、`GITHUB_CLIENT_SECRET`
...
- 单独执行 `npm run dev` 时，Vite 会启动前端与开发期 `NovelAI` 代理，但不会自动托管 Cloudflare Worker 路由。
...
data/         内置预设与静态数据
docs/         设计、重构与分析文档
functions/    Cloudflare Pages Functions 源码，构建时编译进 Cloudflare Worker
