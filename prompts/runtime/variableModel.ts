...
export const 构建变量模型职责提示词 = (options?: { survivalNeedsEnabled?: boolean; cultivationSystemEnabled?: boolean }): string => {
    ...
    return 按功能开关过滤提示词内容(
        格式化多段文本([
            ...
            '8. 每回合都要复核主角与 NPC 档案：主角必须具备 `角色.技艺`，所有 NPC 必须具备当前装备、背包、BUFF、DEBUFF、技艺；技艺至少覆盖炼器、炼丹、医术、阵法、符箓、机关、采集、鉴定等常见项。',
            '9. 技艺不是静态装饰：若正文或变量规划出现学习、试炼、炼制、采集、辨物、治伤、布阵、机关、符箓、丹器等事实，要按故事发展逻辑更新对应角色或 NPC 的技艺等级、熟练度与描述。',
            '10. 若正文或变量规划确认了新地点、镇区、院落、建筑、道路或人物位置变化，要同步维护 `世界.地图层级 / 世界.地图建筑 / 世界.地图道路 / 世界.地图人物`，让地图具备可用的点线面空间结构。',
            ...(survivalNeedsEnabled
                ? [[
                    '11. 生理系统开启时，要把时间推进、休整、进食、饮水、赶路、熬战等事实对应到精力',
                    构建修炼体系附加块('、内力'),
                    '、饱腹、口渴等变量联动。'
                ].join('')]
                : [])
