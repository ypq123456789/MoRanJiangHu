...
const 标准化背景物品列表 = (rawItems: unknown): { 名称: string; 数量?: number; 描述?: string; 类型?: string; }[] | undefined => {
    ...
    const items = rawItems
        ...
        .filter(Boolean) as { 名称: string; 数量?: number; 描述?: string; 类型?: string; }[];
    return items.length > 0 ? items : undefined;
};

const 标准化背景货币列表 = (rawItems: unknown): 背景开局货币[] | undefined => {
    if (!Array.isArray(rawItems)) return undefined;
    const items = rawItems
        .map((item: any) => {
            if (!item || typeof item !== 'object') {
            ...
            const name = 规范化文本(item?.名称);
            if (!name) return null;
            const min = 规范化货币数值(item?.最小数量);
            const max = 规范化货币数值(item?.最大数量);
            return {
                名称: name,
                数量: typeof item?.数量 === 'number' && item.数量 > 0 ? item.数量 : undefined,
                最小数量: min > 0 ? min : undefined,
                最大数量: max > 0 ? max : undefined,
                描述: 规范化文本(item?.描述) || undefined,
                类型: 规范化文本(item?.类型) || undefined,
            };
        })
        .filter(Boolean) as 背景开局货币[];
    return items.length > 0 ? items : undefined;
};
...
const 计算背景开局货币数量 = (entry: 背景开局货币, seed: string): number => {
    if (typeof entry.数量 === 'number' && entry.数量 > 0) return Math.floor(entry.数量);
    const hasMin = typeof entry.最小数量 === 'number' && Number.isFinite(entry.最小数量) && entry.最小数量 >= 0;
    const hasMax = typeof entry.最大数量 === 'number' && Number.isFinite(entry.最大数量) && entry.最大数量 >= 0;
    const min = 规范化货币数值(entry.最小数量);
    const max = 规范化货币数值(entry.最大数量);
    if (hasMin || hasMax) {
        const lo = hasMin ? min : max;
        const hi = hasMax ? max : min;
        return 稳定区间整数(seed, lo, hi);
    }
    return 1;
};

const 构建背景开局货币物品 = (entry: 背景开局货币, params: { seed: string }): any[] => {
    const name = 规范化文本(entry?.名称);
    if (!name) return [];
    const type = 规范化文本(entry?.类型);
    const count = 计算背景开局货币数量(entry, `${params.seed}|${name}|${type}`);
    return [构建背景货币实体物品({
        名称: name,
        数量: count,
        描述: 规范化文本(entry?.描述) || '背景明确派发的开局货币。',
        类型: type || '货币'
    })];
};
...
const 是否唯一剧情道具 = (item: any): boolean => {
    ...
    if (['武器', '防具', '饰品', '消耗品', '材料', '秘籍'].includes(type)) return false;
    const name = 规范化文本(item?.名称);
    if (无限流支线剧情资源正则.test(name)) return false;
    if (item?.类型 === '任务道具') return true;
    if (item?.视觉唯一性 === '唯一' || item?.视觉唯一性 === '主线') return true;
    if (['任务奖励', '支线奖励', '主线奖励', '主线事件'].includes(规范化文本(item?.物品来源类型))) return true;
    return false;
};
...
const 规范化角色物品容器映射 = (rawRole?: any, options?: 玩家BUFF规范化选项): 角色数据结构 => {
    ...
    if (图片档案) {
        ...
        const hadArchive = Boolean(role?.图片档案);
        const hadRecent = Boolean(role?.最近生图结果);
        if (hadArchive || hadRecent) {
            console.warn('[状态规范化] 图片档案合并后为空，保留原始档案避免丢失', {
                角色名: (role as any)?.姓名 || 'unknown',
                hadArchive,
                hadRecent,
                archiveKeys: hadArchive ? Object.keys(role.图片档案 || {}) : [],
                recentStatus: (role as any)?.最近生图结果?.状态 || 'none',
                recentHasUrl: Boolean((role as any)?.最近生图结果?.图片URL),
                recentHasPath: Boolean((role as any)?.最近生图结果?.本地路径)
            });
        } else {
    ...
    // 若出身背景定义了必投资产，使用背景资产并跳过系统自动预设（仅首次生效）
    const 背景初始物品 = (role as any)?.出身背景?.初始物品;
    const 背景开局货币 = (role as any)?.出身背景?.开局货币;
    const 背景接管开局资产 = Array.isArray(背景初始物品) || Array.isArray(背景开局货币);
    const 是否已补过背景物 = (role as any).已补齐系统丹药预设 === true;
    let 本轮背景资产已落地 = false;
    if (背景接管开局资产 && !是否已补过背景物) {
        const 背景物品 = (Array.isArray(背景初始物品) ? 背景初始物品 : [])
            .map((entry: any) => {
                const name = 规范化文本(entry?.名称);
                const type = 规范化文本(entry?.类型);
                if (!name) return null;
            ...
            .flat()
            .filter(Boolean);
        const 背景货币物品 = (Array.isArray(背景开局货币) ? 背景开局货币 : [])
            .map((entry: 背景开局货币) => 构建背景开局货币物品(entry, {
                seed: [role?.姓名, role?.出身背景?.名称, role?.出身背景?.描述].map((value) => 规范化文本(value)).join('|')
            }))
            .flat()
            .filter(Boolean);
        // 用背景必投资产替换当前物品列表（不含自动预设丹药和默认货币）。
        role.物品列表 = 合并同名可堆叠物品([...背景物品, ...背景货币物品]);
...
/** 多货币汇率系统：将货币桶中已废弃的单位名迁移到新 ID（通过 aliases 匹配） */
...
const 标准化单个NPC = (rawNpc: any, fallbackIndex: number): any => {
    ...
    return {
        ...
        ...(首次亲密记录 ? { 首次亲密记录 } : {}),
        ...(上次更新时间 ? { 上次更新时间 } : {}),
        ...(图片档案 ? { 图片档案, 最近生图结果: 图片档案.最近生图结果 } : (npc?.图片档案 ? { 图片档案: npc.图片档案, 最近生图结果: npc.最近生图结果 } : {}))
    };
};
