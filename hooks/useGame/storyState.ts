...
export const 同步角色与门派状态 = <T extends { 角色?: any; 玩家门派?: any }>(state: T): T => {
    ...
    // [修复] 确保活跃门派的重要成员包含主角本人。
    // 变量生成/runtimeVariableWorkflow 会过滤掉 AI 误写的主角 NPC 化条目（符合提示词 21.2），
    // 但过滤后没有把主角作为 是否玩家本人: true 的成员加回去，
    // 导致双人小队轮回者人数只有队友数（1人），主角被漏算。
    const playerMember = 创建玩家门派成员简报(role, sect.名称, sect.玩家职位);
    if (playerMember) {
        sect = { ...sect, 重要成员: 合并玩家到重要成员(sect.重要成员, playerMember) };
    }
...
const 创建默认同门名录 = (sectName: string, openingConfig?: OpeningConfig, playerName?: string): 详细门派结构['重要成员'] => {
...
export const 创建开局门派状态 = (
    ...
    const normalized = 规范化门派状态({
        ...
        任务列表: [],
        重要成员: 合并玩家到重要成员(创建默认同门名录(baseName, openingConfig, charData?.姓名), playerMember)
    });
    return normalized;
};
