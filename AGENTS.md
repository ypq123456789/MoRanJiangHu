...
## 2026-09-14 Relay Early-Stream Against Cloudflare 524, And Stream Error Frames (unreleased)

- Follow-up player report: after v1.0.667 the same player (using an **Ollama cloud** model, not local) still failed, now with `API Error: 524 - error code: 524` during world generation. Local-vs-cloud matters: a cloud Ollama endpoint is publicly reachable on 443 and therefore relay-capable, unlike a local `127.0.0.1:11434` instance.
- Root cause: the relay did `const upstream = await fetch(...)` and only then `return new Response(upstream.body)`. Cloudflare's edge aborts the client connection when the origin has not produced **response headers** within 100 seconds and substitutes its own 524 page (whose body is literally `error code: 524`). So whenever the upstream's first byte was slower than 100s, the relay silently converted a slow-but-working model into an unreadable gateway error — precisely for the players whose only route is the relay.
- Fix in `functions/api/ai-relay/[[path]].ts`:
  - Streaming requests only (`Accept: text/event-stream`, or `"stream":true` in the body) now `Promise.race` the upstream fetch against `EARLY_STREAM_DEADLINE_MS = 80_000`. If the deadline wins, the relay immediately returns `200` + `text/event-stream` with header `X-MSJH-Relay-Early: 1`, opening the SSE pipe before Cloudflare's 100s limit can fire, and then pipes the upstream body through a `TransformStream` once it arrives.
  - The drain task is registered with the Pages Functions `waitUntil` so the runtime does not recycle the isolate the moment the handler returns; without it the pipe closes as an empty stream.
  - If the upstream ultimately fails, the relay writes `data: {"error":{"message":"API Error: <status> - <detail>"}}` followed by `data: [DONE]`, so the failure reaches the client as data rather than as silence.
  - Non-streaming requests deliberately keep awaiting the upstream and never take the early path: returning an SSE body would break their JSON parsing.
- Client side (`services/ai/chatCompletionClient.ts`):
  - `创建SSE文本处理器.尝试解析JSON并提取` now **throws** `协议请求错误` when a frame carries `error` (string or `.message`). Previously these frames were silently ignored, so a relay-reported failure surfaced to the player as an empty reply.
  - New exported `翻译网关超时提示(status, detail)` turns 524/504 into actionable Chinese text (gateway timeout, 100s with no upstream output, suggest a faster model / shorter input / retry) while keeping a trimmed raw fragment for triage, and is applied in both the XHR `onload` non-2xx branch and the fetch non-ok branch.
  - 524 is intentionally **not** auto-retried: the gateway already answered, so a resend risks duplicate generation and duplicate billing. `疑似浏览器跨域失败` returns false when the error carries a status, so the relay is not re-attempted either.
- Remaining, deliberate limitation to communicate honestly: the relay's early return only removes **Cloudflare's** 100s ceiling. The client's own first-response watchdog still applies — main story uses `首次响应超时秒` (default 120s), while world generation uses a 300s idle budget (`执行带超时(..., { idleTimeout: true })`, reset by each streamed delta). A model that needs longer than the caller's budget will still be aborted; that is a settings/product decision, not a relay bug.
- Tests: `__tests__/aiRelayCors.test.ts` gained two cases (early stream past 80s returns 200 + `X-MSJH-Relay-Early` and hands the drain to `waitUntil`, then surfaces the upstream 524 as a readable SSE error frame; a non-streaming request past the deadline must *not* return early) and `__tests__/aiRelayStreamFallback.test.ts` gained 网关超时 / 流内错误帧 cases plus a `翻译网关超时提示` block.
- Suite note: `npm run test:run` reports failures only from `.release-repo-v634/**` (a stale backup tree that vitest still collects, including its own `node_modules`) and from real-AI E2E specs returning HTTP 403 `PERMISSION_DENIED` (Gemini licence). Neither is related to source changes; treat the non-E2E unit suite as the signal.
