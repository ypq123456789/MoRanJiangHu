...
const FlatBar: React.FC<{ label: string; current: number; max: number; type: 'stamina' | 'inner' | 'food' | 'water' | 'load' | 'exp'; visualConfig?: 视觉设置结构 }> = ({ label, current, max, type, visualConfig }) => {
    ...
    const 标题字号 = Math.max(12, Math.round((Number(areaStyle.fontSize) || 13) * 0.92));
    const 数值字号 = Math.max(11, Math.round((Number(areaStyle.fontSize) || 13) * 0.88));
...
const MiniBodyPart: React.FC<{ name: string; current: number; max: number; status: string; visualConfig?: 视觉设置结构 }> = ({ name, current, max, visualConfig }) => {
    ...
    const areaStyle = 构建区域文字样式(visualConfig, '左侧栏');
    const 名称字号 = Math.max(12, Math.round((Number(areaStyle.fontSize) || 13) * 0.88));
    const 数值字号 = Math.max(11, Math.round((Number(areaStyle.fontSize) || 13) * 0.82));
...
const LeftPanel: React.FC<Props> = ({ 角色, onOpenCharacter, onUploadAvatar, visualConfig, gameConfig }) => {
    ...
    const 缩放字号 = (ratio: number, min = 11) => `${Math.max(min, Math.round(基础字号 * ratio))}px`;
    ...
    return (
        <div className="h-full flex flex-col p-3 border-r border-gray-900 relative overflow-hidden bg-transparent" style={areaStyle}>
            ...
            <div className="mb-4 h-[152px] border-b border-gray-800/50 pb-4 shrink-0 flex flex-col items-center justify-center">
                <div className="w-full flex items-center justify-between gap-3">
                    <div className="min-w-0 flex flex-col items-center justify-center flex-1">
                        <div className="w-full text-wuxia-gold/70 italic text-center leading-tight whitespace-nowrap overflow-hidden text-ellipsis" style={{ fontSize: 缩放字号(0.88, 12) }}>{角色.称号 || '无称号'}</div>
                        <button
                            type="button"
                        ...
                        {启用修炼体系 && (
                            <div className="mt-2 bg-wuxia-red/90 border border-red-800/50 text-white px-2 py-0.5 rounded-sm whitespace-nowrap shadow-sm" style={{ fontSize: 缩放字号(0.88, 12) }}>{角色.境界}</div>
                        )}
                    </div>
                        ...
                        <div className="avatar-card-bg">
                            {玩家头像地址 ? (
                                ...
                                <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-b from-white/5 to-black/50 text-wuxia-gold/70 rounded-[8px]">
                                    ...
                                    <div className="mt-1 uppercase tracking-[0.2em] text-gray-500" style={{ fontSize: 缩放字号(0.82, 11) }}>
                                        上传头像
                                    </div>
                                </div>
                            )}
            ...
            <div className="mb-3 shrink-0 border border-gray-800/60 bg-black/30 px-2 py-1.5 flex items-center justify-between font-mono" style={{ color: 'rgba(209,213,219,1)', fontSize: 缩放字号(0.88, 12) }}>
            ...
            <div className="shrink-0 flex flex-col mb-2">
                <div className="border border-gray-800 bg-white/5 p-2 flex flex-col relative group hover:border-gray-700 transition-colors">
                    <h3 className="text-wuxia-gold/70 mb-2 uppercase tracking-[0.2em] text-center bg-black/80 -mt-4 mx-auto px-2 w-fit border border-gray-900 shadow-sm" style={{ fontSize: 缩放字号(0.92, 12) }}>身躯</h3>
            ...
            {玩家BUFF列表.length > 0 && (
                <div className="mb-3 shrink-0 border border-gray-800 bg-white/5 p-2">
                    <h3 className="text-wuxia-cyan/80 mb-2 uppercase tracking-[0.2em] text-center" style={{ fontSize: 缩放字号(0.92, 12) }}>玩家BUFF</h3>
                    <div className="space-y-1.5">
                        {玩家BUFF列表.map((buff, i) => (
                            <div key={`${buff.索引}-${i}`} className="border border-wuxia-cyan/20 bg-black/40 px-2 py-1">
                                <div className="flex items-center justify-between gap-2" style={{ fontSize: 缩放字号(0.88, 12) }}>
                                    <span className="text-wuxia-cyan truncate">{buff.名称 || `BUFF${i + 1}`}</span>
                                    <span className="text-gray-400 whitespace-nowrap" style={{ fontSize: 缩放字号(0.82, 11) }}>{buff.结束时间 || '常驻'}</span>
                                </div>
                                {buff.效果 && <div className="text-gray-300 mt-0.5 leading-tight" style={{ fontSize: 缩放字号(0.82, 11) }}>{buff.效果}</div>}
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="shrink-0 pt-2 border-t border-gray-800/50 flex-1 overflow-y-auto no-scrollbar">
                <h3 className="text-wuxia-gold/70 mb-2 uppercase tracking-[0.2em] text-center" style={{ fontSize: 缩放字号(0.92, 12) }}>行头</h3>
                <div className="space-y-1">
                    {equipmentOrder.map((item) => (
                        <div key={item.key} className="flex justify-between items-center group cursor-help border-b border-gray-800/30 pb-0.5 last:border-0 hover:bg-white/5 px-1" style={{ fontSize: 缩放字号(0.88, 12) }}>
