...
const MobileDetailMetricCard: React.FC<{ groupTitle: string; entry: any }> = ({ groupTitle, entry }) => (
    <div className="group relative rounded border border-gray-700 bg-black/25 px-2.5 py-2 focus-within:border-wuxia-gold/60">
        ...
        <div className="pointer-events-none absolute bottom-[calc(100%+6px)] right-0 z-[100] hidden w-72 max-w-[calc(100vw-3rem)] rounded border border-wuxia-gold/45 bg-[#14110a] p-3 text-xs leading-5 text-amber-50 shadow-2xl group-hover:block group-focus-within:block">
                    ...
                    <div className="rounded-lg border border-wuxia-gold/10 bg-black/25 px-3 py-2.5">
                        ...
                        {货币物品列表.length > 0 ? (
                            <div className="mt-2.5 border-t border-white/10 pt-2.5">
                                <div className="mb-1.5 text-[10px] font-bold tracking-[0.16em] text-gray-500">货币持有</div>
                                <div className="space-y-1.5">
                                    {货币物品列表.map((entry) => (
                                        <div key={entry.类型} className="rounded border border-white/8 bg-black/20 px-2 py-1.5">
                                            ...
                                            <div className="mt-0.5 text-[10px] text-gray-500">分类：{entry.分类名}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
