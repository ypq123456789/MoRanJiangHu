...
const NewGameWizard: React.FC<Props> = ({ onComplete, onCancel, loading, apiConfig, requestConfirm, isStreamingDefault = true }) => {
    ...
    // 选择池仅展示非隐藏天赋；隐藏天赋仍可被背景自带 resolve
    ...
    useEffect(() => {
        ...
        // 玩家自选过滤用全目录（含隐藏），避免误删；背景自带不写入 selectedTalents
    ...
    const 取消选择天赋 = (名称: string) => {
        // 背景自带不可从玩家自选层取消（也不应在 selectedTalents 中）
                        ...
                        <div className="space-y-8 animate-slide-in max-w-6xl mx-auto">
                            ...
                            <OrnateBorder className="p-6 md:p-7 bg-gradient-to-br from-black/70 via-black/55 to-wuxia-gold/5">
                                ...
                                <div className="mb-5 rounded-2xl border border-wuxia-gold/20 bg-black/35 px-4 py-4 shadow-[inset_0_0_20px_rgba(212,175,55,0.06)]">
                                    ...
                                    {当前背景自带天赋.length > 0 && (
                                        <div className="mt-3 rounded-xl border border-rose-400/25 bg-rose-950/20 px-4 py-3 text-sm text-rose-100/90 leading-7">
                                    ...
                                    <div className="flex flex-col items-start md:items-end gap-3">
                                        <div className="flex flex-wrap gap-2 justify-start md:justify-end">
                                            {当前背景自带天赋.map((item) => (
                                                <span
                                                    key={`builtin-${item.名称}`}
                                                    className="rounded-full border border-rose-400/40 bg-rose-950/30 px-3 py-1 text-xs text-rose-100"
                                                    title="背景自带，不可取消"
                                                >
                                                    {item.名称} · 自带
                                                </span>
                                        ...
                                        <div className="text-[11px] text-gray-500">自选 {selectedTalents.length}/3 个（背景自带不占名额），天赋更偏向长期成长路线。</div>
                                        <button
                                            onClick={() => {
                                                if (showCustomTalent) {
                                                    重置自定义天赋编辑();
