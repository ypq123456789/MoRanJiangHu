                                    ...
                                    <div className="lg:col-span-7 space-y-6">
                                        ...
                                        <div className="bg-black/40 p-5 border border-sky-900/30 rounded-xl relative overflow-hidden shadow-lg">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                ...
                                                <div className="rounded-lg border border-white/10 bg-black/30 p-3">
                                                    <div className="mb-2 text-[10px] tracking-[0.2em] text-wuxia-gold/70">背包</div>
                                                    <div className="flex flex-wrap gap-1.5">
                                                        {读取NPC背包(currentNPC).length > 0 ? 读取NPC背包(currentNPC).map((item, idx) => (
                                                            <span key={`${item.名称}-${idx}`} className="rounded border border-gray-700 bg-black/40 px-2 py-1 text-[10px] text-gray-300">
                                                                {item.名称}{item.数量 && item.数量 > 1 ? ` x${item.数量}` : ''}
                                                            </span>
                                                        )) : <span className="text-xs text-gray-600">暂无物品</span>}
                                                    </div>
                                                </div>
                                                <div className="rounded-lg border border-white/10 bg-black/30 p-3">
                                                    <div className="mb-2 text-[10px] tracking-[0.2em] text-emerald-300/80">BUFF / DEBUFF</div>
                                                    <div className="space-y-1.5 text-[10px]">
                                                        ...
                                                        {读取NPC状态(currentNPC, 'BUFF').length + 读取NPC状态(currentNPC, 'DEBUFF').length <= 0 ? <span className="text-xs text-gray-600">暂无状态</span> : null}
                                                    </div>
                                                </div>
                                                <div className="rounded-lg border border-white/10 bg-black/30 p-3">
