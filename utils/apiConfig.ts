...
const 默认词组转化器提示词预设列表: 词组转化器提示词预设结构[] = [
    ...
    },
    {
        ...
        类型: 'npc',
        提示词: [
            '你是 ComfyUI NPC 角色提示词整理器。参考 ComfyUI Prompt Assistant 的预设插入、标签整理、提示词扩写思路，把 NPC 资料整理成稳定、可复用、可直接进入 ComfyUI 工作流的英文提示词。',
            '输出应兼容常见 SDXL / Pony / Illustrious / 写实系 ComfyUI 工作流：优先使用清晰英文短语与必要 tags，不使用过度依赖单一平台的私有语法。',
        ...
        角色锚定模式提示词: [
            '角色稳定外观已经由锚点提供。请沿用锚点中的五官、发型、体型、常驻服装、主要配饰和身份道具。',
...
const 获取已发现ComfyUI后端 = (): Array<{ url: string }> | null => {
    if (已发现ComfyUI后端缓存 && Date.now() - 已发现ComfyUI后端缓存时间 < 已发现ComfyUI后端缓存有效期) {
        return 已发现ComfyUI后端缓存;
    }
    return null;
...
export const 获取已发现ComfyUI后端候选 = (): Array<{ url: string }> => {
