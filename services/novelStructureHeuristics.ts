...
const 强章节标题规则 = [
    new RegExp(`^(?:正文\\s*)?(?:第\\s*${中文数字片段}\\s*${卷级单位片段}\\s*${标题分隔符片段}+\\s*)?(?:第\\s*${中文数字片段}\\s*${章级单位片段}|${非歧义裸章级单位片段}\\s*${中文数字片段})[^\\n]{0,40}$`, 'u'),
    new RegExp(`^(?:正文\\s*)?第\\s*${中文数字片段}\\s*${章级单位片段}[^\\n]{0,40}$`, 'u'),
    new RegExp(`^(?:正文\\s*)?${非歧义裸章级单位片段}\\s*${中文数字片段}[^\\n]{0,40}$`, 'u'),
    /^(?:正文\s*)?话\s*[0-9０-９]+[^\n]{0,40}$/u,
    new RegExp(`^(?:chapter|chap)\\s*(?:${英文章节序号片段})\\b[^\\n]{0,40}$`, 'iu'),
    new RegExp(`^(?:正文\\s*)?\\d{1,5}\\s*[、.．。:：\\-—]\\s*[^\\s].{0,36}$`, 'u')
];
const 强卷标题规则 = [
...
const 去掉单段标题前缀 = (value: string): string => {
    ...
    while (changed && current) {
        ...
        const strippedDelimiter = current.replace(/^[｜|:：·•、.．。\-—/_ ]+/u, '').trim();
        if (strippedDelimiter !== current) {
            current = strippedDelimiter;
            changed = true;
        }
    }
    return current.trim();
