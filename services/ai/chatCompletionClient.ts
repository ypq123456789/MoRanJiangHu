    ...
    for (let pass = 0; pass < 3; pass++) {
        ...
        if (useStream && 支持XHR流式请求()) {
            ...
            try {
                ...
                //
                // 已知取舍（审查意见 P1）：浏览器因"响应缺少 CORS 响应头"而拦截时，
                // responseText 为空并不能证明上游没处理过该请求，理论上可能重复生成一次。
                // 这里保留重试，原因有三：
                //   ① 该场景要求服务端预检通过、POST 响应却缺 CORS 头（自身不一致的配置），
                //      而绝大多数失败是预检/混合内容拦截，POST 根本没发出；
                //   ② 一旦中转成功即返回，最多重复一次，不会随重试次数放大；
                //   ③ 非流式路径（fetchWithCorsRelay）早已是同样的取舍，此处至少多了
                //      "零输出才重发"这一层额外保护。
                // 不重试则网页版对无 CORS 的接口彻底不可用，代价更大。
