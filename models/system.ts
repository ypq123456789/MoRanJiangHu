...
export type 接口供应商类型 = 'gemini' | 'claude' | 'openai' | 'xai_official' | 'deepseek' | 'zhipu' | 'mimo_api' | 'mimo_token_plan' | 'openai_compatible';

export type OpenAI兼容方案类型 = 'custom' | 'siliconflow' | 'together' | 'groq';

export type 请求协议覆盖类型 = 'auto' | 'openai' | 'gemini' | 'claude' | 'deepseek' | 'glm';
export type 主剧情消息模式类型 = 'Gemini模式' | 'GPT' | 'DeepSeek标准' | 'DeepSeek锁格式' | 'GLM标准' | 'GLM锁格式';
export type DeepSeek开局策略类型 = '禁止开局' | '标准开局' | '锁头开局';
export type GLM开局策略类型 = '禁止开局' | '标准开局' | '锁头开局';
...
export interface 游戏设置结构 {
    ...
    GLM策略: GLM策略结构; // GLM main-story stability strategy
    ...
    启用严格正文对白格式?: boolean; // Validate narrator/dialogue line format before accepting response
...
export interface 记忆配置结构 {
    ...
    剧情回忆检索基础超时秒数: number; // 默认 25（首次基础超时）
    剧情回忆检索每10回合追加秒数: number; // 默认 6（每 10 回合追加超时，反映上下文增长）
    剧情回忆检索最大超时秒数: number; // 默认 180（动态追加的上限）
    短期转中期提示词: string; 
    中期转长期提示词: string;
    NPC记忆总结提示词: string;
}
