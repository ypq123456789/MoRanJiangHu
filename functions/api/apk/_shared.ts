...
export const buildB2PublicApkUrl = (env: any, fileName: string): string => {
    ...
    return buildB2PublicApkUrlFromKey(env, `${prefix}/${fileName}`);
};

const buildB2PublicApkUrlFromKey = (env: any, key: string): string => {
    const baseUrl = readB2CdnBaseUrl(env);
    const prefix = readB2ReleaseObjectPrefix(env);
    const normalizedKey = normalizeObjectKey(key);
    const prefixedRoot = `${prefix}/`;
    const relativePath = normalizedKey.startsWith(prefixedRoot)
        ? normalizedKey.slice(prefixedRoot.length)
        : normalizedKey.split('/').filter(Boolean).pop() || normalizedKey;
    return `${baseUrl}/public/${encodeS3Path(prefix)}/apk/${encodeS3Path(relativePath)}`;
};

/**
 * Build the public CDN Worker redirect for a B2-backed APK object key.
 */
export const buildB2ApkRedirect = async (
    ...
    const publicUrl = buildB2PublicApkUrlFromKey(env, key);
